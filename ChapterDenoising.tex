...
\left< P^{(1)}_{ia} \right> &=& f_i(\vec{x}_a)P^{(0)}_a \\
...
&& {} + L_i(\vec{x}_a,t_a) G^E_i(t) E_a \delta_{ij} \delta_{ab} \left[\left(0.1 + 1.9\right)G^P_i(t) + \frac{\sigma^2_{G_i}(t)}{G^P_i(t)}\right].
\end{IEEEeqnarray}
\begin{comment}
...
Outline new algorithm demonstrating multi-threading and batch handling.  AM I REALLY GOING TO DO THIS EXPLICITLY?  SEEMS LIKE KIND OF A MESS.
