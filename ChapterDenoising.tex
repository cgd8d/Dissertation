...
so that the entries alternate between $A$ and $B$, iterating quickly through channels and more slowly through frequencies.  Note that only the $A$ terms are included for the maximum frequency; this is because $X_i[t]$ is real-valued, so $\widetilde{X}_i[0]$ and $\widetilde{X}_i[f_{max}]$ are real-valued as well.
...
In a similar way, we can define the other noise terms in terms of matrix operations.  We will find that it is possible to the noise terms correlated with signals as a product of two matrices, $\mathbf{P = P_1 P_2}$.  First we define the matrix $\mathbf{P_1}$, which steps through the index $b$ horizontally; each column $b$ of $\mathbf{P_1}$ takes the form::
\begin{gather*}
\mathbf{P_1} = \begin{pmatrix}
  E_b \widetilde{Y}^R_{1 b}[1] L_1(\vec{x}_b, t_b) q(1,b) \\
  E_b \widetilde{Y}^I_{1 b}[1] L_1(\vec{x}_b, t_b) q(1,b) \\
  E_b \widetilde{Y}^R_{2 b}[1] L_2(\vec{x}_b, t_b) q(2,b) \\
  \vdots \\
  E_b \widetilde{Y}^I_{i_{max} b}[1] L_{i_{max}}(\vec{x}_b, t_b) q(i_{max},b) \\
  E_b \widetilde{Y}^R_{1 b}[2] L_1(\vec{x}_b, t_b) q(1,b) \\
  \vdots \\
  E_b \widetilde{Y}^R_{1 b}[f_{max}] L_1(\vec{x}_b, t_b) q(1,b) \\
  E_b \widetilde{Y}^R_{2 b}[f_{max}] L_2(\vec{x}_b, t_b) q(2,b) \\
  \vdots \\
  E_b \widetilde{Y}^R_{i_{max} b}[f_{max}] L_{i_{max}}(\vec{x}_b, t_b) q(i_{max},b)
\end{pmatrix} \\
\text{where we define~} q(i,b) := (0.1 + 1.9) G^E_i(t_b) G^P_i(t_b) + \frac{G^E_i(t_b)}{G^P_i(t_b)} \sigma^2_{G_i}(t_b) + L_i(\vec{x}_b,t_b) E_b \frac{\sigma^2_{NU}}{\left(G^P_i(t_b)\right)^2}
\end{gather*}
