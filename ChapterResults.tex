...
It is necessary to perform a number of cuts on the dataset before it is usable.  These cuts are intended to reject events which may not agree with the model used to generate background PDFs.  These cuts include:
\begin{itemize}
\item A fiducial volume cut to remove events with energy deposits outside of a pre-defined 
\end{itemize}


data cuts: fiducial volume, coincidence cuts, diagonal cut.
...
Pdfs included; floating beta scale applied to beta-like.
	Neglects pair-production partial beta scale.

Three observables: energy, standoff, and two-category SS/MS.

Likelihood is used as test statistic; profile is used for most errors.  Important errors (like bb0n magnitude) may be confirmed with toy MC (which lets us check systematics like shape error in spectrum, calibration errors

Efficiency and mass are estimated, also with some error; denoising appears to have full efficiency in the low-background data, but requires a correction in source data due to truncated events.
