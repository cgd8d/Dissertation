...
Pulse-finding proceeds in two steps.  First, on u-wire, v-wire, and summed APD waveforms a matched filter is applied to do a preliminary search on all channels.  Then, a secondary search is performed on the u-wire waveformss to improve sensitivity to multiple pulses near in time.
...
To minimize the impact of pulse templates whose shape may not perfectly relect the shaping times in hardware, and also to reduce the effect which waveform noise may have on our fit, we do not fit the entire waveform to our pulse model.  Instead, we use only an $80 \mu$s fit window in the case of v-wires and APDs, and a $180 \mu$s fit window in the case of u-wires.  The fit windows are illustrated with example events in figure~\ref{fig:ReconExampleFits}.
